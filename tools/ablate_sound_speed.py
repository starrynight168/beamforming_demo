...
class SpeedOfSoundEvaluator:
    ...
    def __init__(
        ...
        # Load H5 dataset
        print(f"[Dataset] Loading H5 file: {self.h5_path} (Sample {self.sample_idx})...")
        (
            self.c_nominal,
            self.fc,
            self.fs,
        ...
        if not self.has_gt or self.gt_data is None:
            raise ValueError(f"H5 dataset {self.h5_path} does not contain multi-angle DAS ground truth (all_envdb_norm)!")
        ...
        if self.gt_display.ndim != 2:
            raise ValueError(f"Expected 2D ground truth display array, got shape {self.gt_display.shape}")
        ...
        print(f"[Imaging] Single-angle selected: index={self.c_idx}, angle={self.single_angle_deg:.2f} deg ({self.single_angle:.4f} rad)")
        print(f"[Transducer] {self.n_elem} channels, fc={self.fc/1e6:.2f} MHz, pitch={self.pitch*1e3:.3f} mm, nominal c={self.c_nominal:.1f} m/s")
        print(f"[Grid] Depth: {self.z_grid[0]*1e3:.1f} ~ {self.z_grid[-1]*1e3:.1f} mm ({len(self.z_grid)} pts), Lateral: {self.x_grid[0]*1e3:.1f} ~ {self.x_grid[-1]*1e3:.1f} mm ({len(self.x_grid)} pts)")
    ...
    def evaluate_c(self, c_val):
        ...
        if not np.isfinite(c_key) or c_key <= 0:
            raise ValueError(f"sound speed must be a finite positive value, got {c_val!r}")
        ...
        with torch.inference_mode():
            i_out, q_out = beamformer(self.sel_i, self.sel_q, self.sel_angles, self.sel_t0, self.fs)
        ...
        if not np.isfinite(peak) or peak <= 0:
            raise ValueError(f"Beamformer output is invalid for c={c_key:.3f} m/s: peak={peak!r}")
...
def multi_section_search(evaluator, c_min=1430.0, c_max=1620.0, num_sections=4, tol=2.0, max_iter=8):
    ...
    print(f"\n{'='*70}")
    print(f"  Starting Multi-Section Bracketed Search (Sections: {num_sections}, Tol: {tol:.1f} m/s)")
    ...
    while (b - a) > tol and iteration < max_iter:
        ...
        for idx, c_val in enumerate(probe_c):
            ...
            print(f"  Point {idx+1}/{len(probe_c)}: c = {c_val:7.2f} m/s | SSIM = {res['ssim']:.6f} | PSNR = {res['psnr']:.2f} dB | MAE = {res['mae_db']:.2f} dB ({res['elapsed_sec']:.2f}s)")

        best_idx = int(np.argmax([r["ssim"] for r in probe_results]))
        best_res = probe_results[best_idx]
        print(f"  -> Best in iteration {iteration}: c = {best_res['c']:.2f} m/s with SSIM = {best_res['ssim']:.6f}")

        history.append({
            "iteration": iteration,
            "interval_before": [a, b],
            "probes": [{"c": r["c"], "ssim": r["ssim"], "psnr": r["psnr"], "mae_db": r["mae_db"]} for r in probe_results],
            "best_c": best_res["c"],
...
def dense_grid_sweep(evaluator, c_min=1430.0, c_max=1620.0, step=10.0):
    ...
    print(f"[Grid Sweep] Scanning {len(grid_c)} points from {c_min:.1f} to {c_max:.1f} m/s (step={step:.1f} m/s)...")
    results = []
    for idx, c_val in enumerate(grid_c):
        res = evaluator.evaluate_c(c_val)
        results.append(res)
        print(f"  [{idx+1:2d}/{len(grid_c):2d}] c = {c_val:7.2f} m/s | SSIM = {res['ssim']:.6f} | PSNR = {res['psnr']:.2f} dB")
    return results
...
def plot_results(evaluator, search_history, overall_best, output_dir):
    ...
    if not all_res:
        raise RuntimeError("Cannot plot results before evaluating at least one sound speed")
    ...
    # Upper panel: SSIM
    ax1.plot(c_list, ssim_list, "o-", color="#1f77b4", linewidth=2.0, markersize=5, label="Single-angle MV vs Multi-angle DAS")
    ax1.axvline(evaluator.c_nominal, color="gray", linestyle="--", alpha=0.7, label=f"Nominal c = {evaluator.c_nominal:.0f} m/s")
    ax1.axvline(overall_best["c"], color="#d62728", linestyle=":", linewidth=1.8, label=f"Optimal c* = {overall_best['c']:.1f} m/s")
    ax1.plot(overall_best["c"], overall_best["ssim"], marker="*", color="#d62728", markersize=14, label=f"Max SSIM = {overall_best['ssim']:.4f}")
    ...
    for i, step in enumerate(search_history):
        ...
        ax1.scatter(probe_cs, probe_ssims, color=col, s=35, zorder=5, label=f"Search Iter {step['iteration']}" if i < 3 else None)

    ax1.set_ylabel("SSIM", fontsize=12, fontweight="bold")
    ax1.set_title("Speed of Sound (SoS) Ablation & Optimization\n(Single-angle MV vs Multi-angle DAS Ground Truth)", fontsize=13, fontweight="bold")
    ax1.grid(True, linestyle="--", alpha=0.6)
    ax1.legend(loc="upper right", framealpha=0.9, fontsize=9)

    # Lower panel: PSNR & MAE
    ax2.plot(c_list, psnr_list, "s-", color="#2ca02c", linewidth=1.8, markersize=4, label="PSNR (dB)")
    ax2.set_ylabel("PSNR (dB)", color="#2ca02c", fontsize=11, fontweight="bold")
    ax2.tick_params(axis="y", labelcolor="#2ca02c")
    ax2.grid(True, linestyle="--", alpha=0.6)

    ax2_twin = ax2.twinx()
    ax2_twin.plot(c_list, mae_list, "^-", color="#ff7f0e", linewidth=1.8, markersize=4, label="MAE (dB)")
    ...
    images_to_show = [
        ("Multi-angle DAS (GT, 75 angles)", evaluator.gt_display * evaluator.dr - evaluator.dr, "Reference (SSIM=1.0)"),
        (f"Single-angle MV (c = {min_res['c']:.0f} m/s)", min_res["db_img"], f"SSIM = {min_res['ssim']:.4f}"),
        (f"Single-angle MV (c = {nom_res['c']:.0f} m/s, Nominal)", nom_res["db_img"], f"SSIM = {nom_res['ssim']:.4f}"),
        (f"Single-angle MV (c* = {overall_best['c']:.0f} m/s, Optimal)", overall_best["db_img"], f"SSIM = {overall_best['ssim']:.4f}"),
        (f"Single-angle MV (c = {max_res['c']:.0f} m/s)", max_res["db_img"], f"SSIM = {max_res['ssim']:.4f}"),
    ]
    ...
    for ax, (title, db_map, metric_text) in zip(axes, images_to_show):
        im = ax.imshow(db_map, cmap="gray", vmin=-evaluator.dr, vmax=0.0, extent=extent, aspect="auto")
        ax.set_title(f"{title}\n[{metric_text}]", fontsize=10, fontweight="bold")
        ax.set_xlabel("Lateral (mm)", fontsize=9)
        ax.set_ylabel("Depth (mm)", fontsize=9)

    cbar = fig.colorbar(im, ax=axes.ravel().tolist(), orientation="vertical", shrink=0.8, pad=0.015)
...
def save_results(evaluator, search_history, overall_best, final_bracket, output_dir):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["c_mps", "ssim_vs_gt", "psnr_db_vs_gt", "mae_db_vs_gt", "elapsed_sec"])
        for r in all_res:
            writer.writerow([f"{r['c']:.3f}", f"{r['ssim']:.6f}", f"{r['psnr']:.4f}", f"{r['mae_db']:.4f}", f"{r['elapsed_sec']:.3f}"])
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write("=" * 60 + "\n")
        f.write(f"Dataset             : {evaluator.h5_path} (Sample {evaluator.sample_idx})\n")
        f.write(f"Imaging Mode        : Single-angle MV ({evaluator.single_angle_deg:.2f} deg, angle_idx={evaluator.c_idx})\n")
        f.write(f"Reference Baseline  : Multi-angle DAS ({len(evaluator.angles)} angles CPWC ground truth)\n")
        ...
        f.write(f"Final Search Bracket: [{final_bracket[0]:.2f}, {final_bracket[1]:.2f}] m/s\n")
...
def main():
    ...
    parser.add_argument("--h5_path", default="data/in_vivo.h5", help="Path to in-vivo H5 dataset")
    parser.add_argument("--sample_idx", type=nonnegative_int, default=0, help="H5 sample index")
    parser.add_argument("--c_min", type=positive_float, default=1430.0, help="Minimum sound speed in search (m/s)")
    parser.add_argument("--c_max", type=positive_float, default=1620.0, help="Maximum sound speed in search (m/s)")
    parser.add_argument("--sections", type=positive_int, default=4, help="Number of sections per bracket iteration")
    parser.add_argument("--tol", type=positive_float, default=2.0, help="Interval tolerance for search termination (m/s)")
    parser.add_argument("--max_iter", type=positive_int, default=6, help="Maximum search iterations")
    parser.add_argument("--grid_step", type=nonnegative_float, default=15.0, help="Step for dense curve sweep (0 to disable)")
    parser.add_argument("--output_dir", default="results/ablation_sound_speed", help="Output directory")
    parser.add_argument("--mv_dl", type=nonnegative_float, default=0.0, help="MV diagonal loading factor")
    parser.add_argument("--fbss", action="store_true", default=True, help="Enable forward-backward spatial smoothing")
    parser.add_argument("--no_fbss", dest="fbss", action="store_false", help="Disable forward-backward spatial smoothing")
    parser.add_argument("--subarray_ratio", type=unit_interval_float, default=0.25, help="MV subarray ratio")
    parser.add_argument("--temporal_win", type=positive_odd_int, default=9, help="Temporal covariance window; must be odd")
    parser.add_argument("--f_number", type=positive_float, default=1.5, help="Receive aperture F-number")
    parser.add_argument("--dynamic_aperture", action="store_true", default=True, help="Enable dynamic receive aperture")
    parser.add_argument("--no_dynamic_aperture", dest="dynamic_aperture", action="store_false", help="Disable dynamic receive aperture")
    parser.add_argument("--tgc", action="store_true", default=True, help="Enable time-gain compensation")
    parser.add_argument("--no_tgc", dest="tgc", action="store_false", help="Disable time-gain compensation")
    parser.add_argument("--tgc_alpha", type=nonnegative_float, default=0.5, help="TGC attenuation coefficient")
    parser.add_argument("--window", choices=WINDOW_CHOICES, default="rect", help="Receive aperture window")
    parser.add_argument("--interp", choices=INTERP_CHOICES, default="linear", help="Channel interpolation method")
    parser.add_argument("--dr", type=positive_float, default=60.0, help="Display dynamic range (dB)")
    args = parser.parse_args()

    try:
        validate_search_config(args.c_min, args.c_max, args.sections, args.tol, args.max_iter)
    ...
    if args.grid_step > 0:
        dense_grid_sweep(evaluator, c_min=args.c_min, c_max=args.c_max, step=args.grid_step)
        overall_best = best_cached_result(evaluator)

    # 3. Output Figures and Data
    sample_name = "carotid_cross" if args.sample_idx == 0 else f"carotid_sample_{args.sample_idx}"
