...
# ---- MV参数（用于自动生成基线） ----
parser.add_argument('--mv_dl', type=float, default=0.1, help='MV 对角加载系数（自动生成基线用）')
...
def ensure_mv_baseline(baseline_path):
    ...
    cmd = [
        ...
        '--h5_path', resolve_project_path(args.h5_path),
...
def main():
    ...
    c, fc, fs, pitch, n_elem, angles_all, t0_all, z_grid, x_grid, I_data, Q_data, gt_data, has_gt = load_from_h5(
        resolve_project_path(args.h5_path), args.h5_sample_idx
    )
