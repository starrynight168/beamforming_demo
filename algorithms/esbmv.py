...
def print_physical_summary(c, fc, fs, pitch, n_elem, angles, t0, H, W, dz, dx,
    ...
    if args.num_eig > 0:
        print(f"  Signal eigs  : {args.num_eig}")
    else:
        print(f"  Eig threshold: {args.eig_threshold}")
        ...
        for i in range(n_a):
            ...
            for hz in range(self.H):
                ...
                if args.interp == 'nearest':
                    ...
                    I_samples = I_angle[idx0, self.ch] * (1.0 - frac) + I_angle[idx0 + 1, self.ch] * frac
                    Q_samples = Q_angle[idx0, self.ch] * (1.0 - frac) + Q_angle[idx0 + 1, self.ch] * frac
                ...
                # ========== 对角加载 ==========
                trace = R_signal.diagonal(dim1=-2, dim2=-1).real.sum(-1)  # [W]
                R_dl = R_signal + (self.dl_factor / L) * trace.view(self.W, 1, 1) * row['eye']
                ...
                if args.num_eig > 0:
                    n_eig = max(1, min(args.num_eig, L))
                    signal_space = eigvecs[:, :, :n_eig]
                    w = signal_space @ (signal_space.mH @ w_mv)
                else:
                    keep = eigvals >= args.eig_threshold * eigvals[:, :1].clamp_min(1e-12)
                    keep[:, 0] = True
